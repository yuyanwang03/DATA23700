...
function findBinForValue(bins, value) {
    ...
    // If value is exactly equal to the upper bound of the last bin
    if (value == bins[bins.length - 1].x1) {
      return bins[bins.length - 1];
    }
    return null; // Return null if the value doesn't fall into any bin
}

// expects prefiltered data
function getBarData(filteredData) {
    console.log("Get Bar DAta");
    let returnData = [];

    filteredData.forEach((obj) => {
        let idx = findBinForValue(bins, obj.hp);
        console.log("BIINS: ");
        console.log(idx);
